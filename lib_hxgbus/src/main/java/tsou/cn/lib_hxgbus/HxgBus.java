...
public class HxgBus {
    ...
    private final Map<Object, List<String>> typesByTagSubscriber;

    private HxgBus() {
        typesBySubscriber = new HashMap<>();
        typesByTagSubscriber = new HashMap<>();
    ...
    public void register(Object object) {
        ...
        for (Method method : methods) {
            Subscriber subscribe = method.getAnnotation(Subscriber.class);
            if (subscribe != null) {
                /**
                 * 所有的Subscribe属性 解析出来
                 */
                ...
                SubscriberMethod subscriberMethod = new SubscriberMethod(
                        method, parameterTypes[0],subscribe.tag(), subscribe.threadMode(),
                        subscribe.priority());
    ...
    private void subscriber(Object object, SubscriberMethod subscriberMethod) {
        ...
        if (TextUtils.isEmpty(tag)){
            ...
            CopyOnWriteArrayList<Subscription> subscriptions = subscriptionsByTag.get(tag);
            if (subscriptions == null) {
                subscriptions = new CopyOnWriteArrayList<>();
                subscriptionsByTag.put(tag, subscriptions);
            }
            //判断优先级,同时存储方法名
            Subscription newSubscription = new Subscription(object, subscriberMethod);
            int size = subscriptions.size();
            ...
            // typesByTagSubscriber 方便移除
            List<String> strings = typesByTagSubscriber.get(object);
            if (strings == null) {
                strings = new ArrayList<>();
                typesByTagSubscriber.put(object, strings);
            }
    ...
    private void removeString(String string, Object object) {
        CopyOnWriteArrayList<Subscription> subscriptions = subscriptionsByTag.get(string);
        if (subscriptions != null) {
            int size = subscriptions.size();
            for (int i = 0; i < size; i++) {
                Subscription subscription = subscriptions.get(i);
    ...
    public void post(String tag,Object event) {
        //遍历 subscriptionsByTag，找到符合的方法调用方法的 method.invoke()执行，并判断是哪个线程
        //找到符合的方法调用方法的 method.invoke()执行
        CopyOnWriteArrayList<Subscription> subscriptions = subscriptionsByTag.get(tag);
        if (subscriptions != null) {
            for (Subscription subscription : subscriptions) {
